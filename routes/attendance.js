...
router.post("/", upload.single("tiket"), asyncHandler(async (req, res) => {
  const { company, name, kehadiran } = req.body;
  const tiket = req.file ? req.file.filename : null;

  if (!company || !name || kehadiran == null) return res.status(400).json({ error: "Missing fields" });

  const kehadiranInt = Number(kehadiran);
  if (!Number.isInteger(kehadiranInt)) return res.status(400).json({ error: "Kehadiran must be integer" });
...
router.get("/:id/tiket", asyncHandler(async (req, res) => {
  const { id } = req.params;
  const result = await pool.query(
